 ...
 * shape. The server's allow-list is what makes sending a `return_to` at all
 * safe, and an app that sent anything else would simply be refused with a 400.
 */
export const APP_RETURN_TO = 'almosthadai://auth';

/**
 * The app's own version, for the auth screens' bottom line and the account
 ...
 * one it was written for. A version the app reports wrongly is worse than none,
 * because a bug report would name a build that never existed.
 *
...
/** The bare number, for anywhere the word "Version" precedes it. */
export const APP_VERSION_NUMBER: string =
    typeof __APP_VERSION__ === 'string' ? __APP_VERSION__ : '0.0.0-dev';

/**
 * The prefixed form, with a leading `v`, that the auth screens print on their own.
 *
