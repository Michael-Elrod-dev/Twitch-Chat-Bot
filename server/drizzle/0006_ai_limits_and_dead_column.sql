...
-- once, because that row is one the session already reads through a cache it
-- already invalidates on write, so a change takes effect without a restart and
-- without a new notification path to forget to call.
--
...
-- column, so rendering that sentence from a map living in the desktop client
-- means a built-in added on the server shows a generic placeholder until
-- somebody edits that file.
--
-- The description travels with the handler's registration instead, and is
-- written onto the row at session load, exactly as `user_level` already is. The
...
-- ---------------------------------------------------------------------------
-- 3. `streams.unique_chatters`, dropped rather than backfilled.
--
-- Nothing has ever written it, so the column reads 0 for every stream ever
-- recorded, including imported history. The dashboard refuses to read it, and
-- the analytics streams table would have been the second query to trip over the
-- same zero.
--
-- Dropping rather than backfilling, because the honest source is
-- `chat_messages`, which is where both readers count distinct chatters from. A
-- column that lies is worse than an absent one, since an absent one cannot be
-- picked up in good faith by the next query that needs it.
--
-- Nothing is lost. There is no data here to lose, because every row holds the
-- default.
