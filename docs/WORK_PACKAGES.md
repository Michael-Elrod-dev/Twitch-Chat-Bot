...
> **Deferred observation 1 closed 2026-08-17:** the owner installed the dispatch-run artifact — the first CI-built installer proven end to end (download → install → sign-in → live production data; the URL guard ran green in its job) — and the healthy `2b` strip read exactly as designed: SESSION Idle · TWITCH Connected · SPOTIFY Connected · AI On · DISCORD Not set up, dim still dots, connected chat dot (no "reconnecting…" — the reconnect fix confirmed on the owner's machine), `REQUESTS OPEN` pill, both empty-state copies verbatim, OFFLINE pill and active Bot-on switch correct. **No last-stream caption and zero numbers — correct, not a defect:** no `streams` row exists yet (the bot went live after the owner's last stream), so the route's deliberate no-stream fallback renders zeroes and no caption. The caption, real totals, and the uptime tick all get their incidental confirmation at the owner's first stream.
...
### Completion report — P1-WP9b (engineer, 2026-08-16)

**Status: code delivered, deployed, live proof pending the owner.** Server **852/852** (48 files) · app **271/271** (12 files) · lint 0 · `npm run typecheck` exit 0 · production deployed and healthy. Reintroductions **#87–#98**, all biting.

**The mod-role marker.** `ChatRole = 'broadcaster' | 'moderator' | 'vip' | 'viewer'` on the `chat.message` chatter, contract-first, exposure rather than derivation: `chatRoleOf()` sits directly beneath `rankOf()` in `permissions.ts` and reads the same flags in the same precedence, so the feed cannot disagree with what the pipeline actually permitted. Deliberately **not** `UserLevel` — that type answers "who may run this command" and includes `everyone`, a threshold rather than anything a person holds; this answers "what is this person", where the floor is an ordinary viewer. They order alike and mean different things, and the contract says so where a future reader will unify them.

9a's chat feed now colours from the event rather than by comparing a login against the channel's — which could identify the broadcaster and nobody else. **Moderators render sage the moment this ships**, closing 9a's deferred item. Proven end-to-end rather than at the mapping: five tests drive real EventSub badge payloads through the webhook and assert the role that arrives on the socket, including the two cases a unit test would have missed — the broadcaster derived from their **id** (their own messages do not reliably carry the badge, so the owner's lines would otherwise render as ordinary viewers in their own dashboard) and mod-outranks-VIP.

**Validation mirrors by construction, not by agreement.** The brief asks that "the schema and the UI agree, or a test fails". The stronger form was available and taken: the forms **run** `commandNameSchema`, `chatTextSchema`, `emoteTriggerSchema` and `createQuoteSchema` rather than restating them, so there is no second copy to drift. Lowercasing comes free from the schemas' own `.transform`, and the form submits the schema's **output** — `!SOCIALS` is stored as `!socials`, which matters because the chat pipeline looks commands up lowercased and a raw-submitted name would be saved and never matched. The reply counter's `500` is read off `chatTextSchema.maxLength`, not typed; a test asserts the derivation resolved a real limit rather than silently falling through its `??`.

This is the same lesson as the two defects this run has already paid for — the reconciler fixture that agreed with the code and not with Twitch, and the redemption condition that agreed with itself and not the wire. Two things that look alike are not a mirror; one thing asked twice is.

**Commands (`2c`/`4d`/`4e`).** Handler-backed rows are structurally not editable: no `responseText` to change, no route that would accept one, so the row carries a `BUILT IN` chip, its behaviour in the reply column, a lock where the actions go, and **no edit control at all** — a disabled pencil would imply a permission problem, and there is no permission involved. Hover-revealed actions on static rows, footer pagination, All/Yours/Built-in chips, search across behaviour text as well as names. The editor cannot rename an existing command, because `updateCommandSchema` accepts only `responseText` and `userLevel` and the name is the identity in the route path; offering the field would be offering something the API cannot do.

**Emotes (`3a`).** Composer as the first row of the grid, on its own clay wash — adding an emote is a two-field act and a dialog around it would be ceremony. No edit path, matching both the design and the API. Footnote verbatim.

**Quotes (`3b`).** Two-column grid, gaps visible by design, footnote verbatim. The optimistic row carries a placeholder number and the real one arrives on reload: guessing `max + 1` would be wrong the instant two quotes race.

**One error presentation, in one function.** `presentError` decides only *where* a message goes — field for `bad_request`/`conflict`, inline for `rate_limited` (with the `Retry-After` seconds, and deliberately not in the warning colour: being asked to wait is the system working, and dressing it as an error teaches the owner their bot is broken when it is busy), banner for everything else. Nothing anywhere decides to stop rendering; the handoff's rule is that there is never a full-page wall over a working bot.

**A defect the tests found in the design of my own screen.** With none of your own commands, clicking "Built in" still showed the `4d` empty state — a card saying "you have not written any yet" over a list the user had just explicitly asked for. Fixed; and the test for the fix was written only after the reintroduction pass showed #97 passing anyway, which is the honest order to admit to.

**Design fidelity verified in a real browser**, as in 9a, against computed styles: commands grid `190px 1fr 130px 96px`, emotes `280px 1fr 60px`, quotes two columns, modal 560px/14px, scrim `rgba(12,11,10,.62)`, composer wash `rgba(200,102,63,.045)`, lock `#413c37`, mod level `#9dbd8c`, selected WHO chip clay tint `.13` on a `.35` clay border in `#e0805a`, `4d` card padding `52px 40px` with mono suggestion chips and the 4-column built-ins grid, footer reading `1–10 of 22` and the header `5 yours · 17 built in` — the design's own numbers, from the owner's real data. No horizontal overflow. **The pass caught one mismatch**: the `BUILT IN` chip was rendering at `rgba(240,235,225,.09)` where the handoff specifies `.07` — one step quieter than an active filter chip, which is the difference between "this is selected" and "this is a label". Fixed. The harness was deleted; no dead source ships.

**Reintroduction validation (#87–#98).**

| # | Defect reintroduced | Caught by |
|---|---|---|
| 87 | `role` dropped from the published event | 4, live end-to-end |
| 88 | a moderator derived as an ordinary viewer | 2 |
| 89 | the UI restates the name rule instead of running the schema | **6**, the mirror |
| 90 | the form submits the raw name rather than the normalised one | 1 |
| 91 | the reply counter hardcodes its limit | 2 |
| 92 | a failed mutation is not rolled back | 2 |
| 93 | a conflict is bannered instead of placed on the field | 1 |
| 94 | rate limiting dressed as an error state | 1 |
| 95 | built-in rows become editable | 1 |
| 96 | the emote trigger is submitted unnormalised | 1 |
| 97 | the empty state hides the built-ins the user asked to see | 1 (see honesty) |
| 98 | an already-deleted row is rolled back into the list | 1 |

**Honesty items.**
- **My reintroduction harness failed first, loudly, and the mechanism that caught it was ours.** The first run of #87/#88 reported `HARNESS-ERROR` because I passed `REQUIRE_DB_TESTS=1` to the subprocess without `TEST_DATABASE_URL` — and the WP4.3 DB-skip reporter refused to let 24 skipped tests read as a pass. That is the fourth time this run the standing lesson has bitten, and the first time an existing guard caught it for me rather than me catching it.
- **A patch of mine silently did nothing and I nearly believed it.** Inserting the moderator colouring test, my anchor string no longer existed (I had renamed that test in 9a), so the edit was a no-op that reported success — the test count stayed at 67 and I only noticed by checking. Every subsequent scripted edit in this package asserts its anchor before writing.
- **`#97` passed anyway on its first run.** I found the empty-state/filter bug because a *different* test failed for the wrong reason, fixed it, and wrote no test for the fix. The pass caught the gap. Sixth such catch across this run.
- **A test fixture collided with production copy.** The commands fixture was named `!discord`, which is also one of `4d`'s three suggestion chips — so "the row is gone after deleting" was unassertable the moment the last row went and the empty state appeared. Renamed to `!links`, with the reason recorded in the fixture.
- **The built-in behaviour descriptions live client-side, and that is a compromise.** The contract carries `handlerName` and nothing else; the server's registry stores a function and a level, no description. The design requires the reply column to describe the behaviour, so `commandCatalog.ts` maps the seventeen real handler names to text. **Cost stated plainly: a built-in added server-side renders a generic fallback until someone edits that file.** A contract addition was outside this package's scope guard, so this is flagged rather than taken.

**Flagged, not fixed.**
- **Built-in descriptions belong with the handler registrations**, so behaviour and description change together. Smallest shape: a `description` beside `level` in `HandlerRegistration`, surfaced on `Command`.
- **`DELETE /api/v1/emotes/:trigger` uses `emoteTriggerSchema.parse`, not `safeParse`** — an invalid trigger throws into the 500 handler instead of answering 400. Unreachable in practice (a trigger longer than 64 chars cannot exist in the table, and an empty one does not match the route), which is why it is noted rather than fixed inside a package whose server scope was one field.
- **Commands and emotes are fetched whole and paged in memory** (`limit=200`). Correct at the owner's 22 rows and for the filter counts, which are facts about the set rather than the page; a channel with hundreds of commands would want the server-side paging the route already supports.
- Carried from 9a and unchanged: `queueLength` contract/wire disagreement and `streams.unique_chatters` → 9c; retention; the localStorage seam → security audit. The design handoff directory remains untracked and is subject to 9c's deletion criterion.

**Deploy.** Standard deliberate procedure, dry-run first. No migration, no new env var — the role marker is a field on an event. Post-deploy: `healthz` 200, `readyz` both probes green, bootstrap `started:1 failed:0`, and **`create:0 remove:0 keep:4`** — 9a's reconciler fix holding across a fresh deploy, which is independent confirmation the churn is dead rather than merely quiet.

**Remaining before close: the live CRUD loop with the owner** — create a command in the app → use it in chat → the bot answers; edit the reply → the bot answers with the new text; delete → silence; add an emote via the composer → trigger it; add a quote → `!quote` returns it. Mod colouring if a real moderator speaks, otherwise deferred to incidental observation alongside the uptime tick and the healthy `2b` strip.

> **P1-WP9b code half verified by lead 2026-08-17 — live CRUD loop pending.** Battery re-proven by the lead's own hands: server **852/852** on the throwaway DB with `REQUIRE_DB_TESTS=1`, app **271/271**, lint 0, root typecheck exit 0. Critical diffs read: `ChatRole` in the contract (deliberately NOT `UserLevel` — "what someone is" vs "what a command requires", with the reasoning in the type's own comment); `chatRoleOf` placed directly beneath `rankOf` so the two precedences cannot drift apart unseen; the pipeline reading the same event flags through the same function ("the feed cannot disagree with what the pipeline actually let this person do"); the validation mirror running the server's own zod schemas with zero restated rules and the 500-counter derived from `maxLength` — the one-thing-asked-twice principle, endorsed as the strongest form of "mirroring". **Reintroduction re-run by the lead personally:** the broadcaster-by-id derivation replaced with naive badge-trust → exactly two tests failed (the unit and the end-to-end badge-payload test — the owner's own lines would have rendered as an ordinary viewer) → restored → 37/37. Production probed: `readyz` green, reconcile steady at `create:0 remove:0 keep:4` on the fresh deploy — the 9a churn fix independently confirmed holding. Endorsements: the `4d`-filter design bug with the #97 test-written-after-the-fix honesty ("which is the honest order"); the `.07`-vs-`.09` BUILT-IN-chip fidelity catch (the browser pass earning its keep a second package running); the scripted-edit-assert-anchor lesson; and the DB-skip reporter catching the engineer's own harness — the first time an existing guard caught the class instead of a person. **Ruling:** the client-side `commandCatalog` descriptions compromise is accepted for this package and routed to 9c as a small contract rider — a `description` beside each handler's registration, served on handler-backed rows, so a future built-in cannot ship with a generic fallback. **Owner decision recorded (2026-08-17): "v1" is defined by the owner as everything through the security audit PLUS further owner-added features to be specified later. Version tags before that are working builds; the "first release" milestone proves the release process, it does not declare v1.** Committed by lead as this entry's commit. Package stays open until the live CRUD loop is witnessed in the owner's chat.

> **Live-loop catch #1 — the app wrote to storage the bot had stopped reading (engineer fix, lead-verified 2026-08-17).** The owner's first created command saved correctly and never fired: the API writes via the repository, but the session's `CommandManager`/`EmoteManager` hold an in-memory map plus a populated Redis hash whose lookup treats "hash exists, field absent" as an authoritative miss (correct, load-bearing hot-path optimisation) — so a row inserted behind them is invisible **permanently**, not briefly: past the TTL the fallback consults the in-memory map, which `loaded` marks good. Fix: a `reloadChannelContent` seam — route records the change, then tells the running session to re-read — wired on create, edit, AND delete for both kinds (a deleted command that keeps answering is worse than one that never starts), reload completing before the response so the client can't win the race against its own bot. **The seam is REQUIRED, deliberately unlike its neighbours:** the defect was not a wiring someone deleted but a connection nobody had made that no type complained about — optional would leave that silently available again; required makes forgetting it a compile error at the composition root. #99's first version was vacuous (the test wired its own seam — the exact shape of the original bug) and was redone at both levels: route by test, composition root by compile error. Engineer's own words, endorsed and preserved: *a route test passed (the row was written) and a pipeline test passed (it answers what it was loaded with) — only asking "does the bot answer a command the API just created" could fail, which is why that's now the test that exists.* **Third package running in which the live proof caught what the whole suite agreed was fine.** Lead verification: 859/859 + 271/271, lint 0, typecheck 0; diffs read (required-seam rationale, before-response ordering, disabled-channel no-op); reintroduction re-run personally — the create-route reload removed → exactly the owner's-bug test failed → restored 7/7; prod healthy on the redeployed fix. Committed by lead. Reintroductions #87–#102. Loop still pending.

> **P1-WP9b CLOSED — live loop witnessed 2026-08-17.** The owner ran the full circuit and reported every step behaving: the created command answered immediately (the reload fix's first real-world exercise), the edited reply came back with the new text, the deleted command fell silent, the composer emote answered on exact match and stayed silent inside a sentence, and `!quote` returned the added quote. Server-side correlation captured by the lead over SSH: five `Chat message sent` lines against the owner's channel inside the loop's two-minute window. The silences are unlogged absences by nature and rest on the owner's first-person report — which is what the live proof is for. Quote-number gaps confirmed rendering; mod-green remains an incidental observation (no moderator spoke). **P1-WP9c issued below — the final tranche.**

---

## P1-WP9c — Songs, analytics & settings (final tranche)  [STATUS: ISSUED 2026-08-17]

**Goal:** the app is feature-complete. The songs page with the full Spotify surface, analytics, all five settings sub-pages, Stream Deck API keys, account and danger zone. On this tranche's close the design handoff is deleted — the "app is built" milestone. (Per the owner's recorded definition, this is still pre-v1: release-process proof, rebrand, security audit, and owner-specified features follow.)

**Scope guard:** `app/`, `server/` (the additions below, contract-first), `shared/`, docs. Design README ids `3c`, `4c`, `3d`, `3e`, `3f`, `5a`, `5b`, `5c` are the acceptance spec; the contract wins where they disagree.

**Lead calls (locked):**
- **Task 0 — the settings-cache audit, fix-first (the reload-fix class, hunted by grep before the owner finds it live):** `PATCH /me` writes settings via the repository while the session's `SettingsService` may cache behind its own back — the exact shape of the commands hole. Audit with evidence; if the class is present anywhere (settings, per-role limits, playlist fields), extend the required-seam pattern to it BEFORE building the screens that edit those values. Reintroduction: a setting changed via the API affects the running pipeline without a restart.
- **Server riders, all contract-first:** the Spotify surface (connection status with account name + connected-since, disconnect endpoint, playlist name/count for the playlist card, playback state for the now-playing card); per-role AI limit editing (the `3e` steppers; the 4.5 counter-threshold/always-on preference may ride if cheap); rewards status for the trust card (the three bot-managed rewards, bound state, from `channel_rewards`); handler `description` beside each registration served on handler-backed rows (kills 9b's client-side catalog fallback); **`queueLength` on `song_queue.updated` resolved deliberately** — wire it or amend the contract, either with a written reason; **`streams.unique_chatters` decided** — wire it or drop it (lead lean: drop, the `viewers.context` precedent, via migration).
- **Requests-playlist UX (owner-elevated product feature):** the `5a` toggle + streamer-named playlist + create-if-missing + skip-gracefully per `PHASE1_DESIGN.md` §3. The append+dedup foundation exists from 4.3; this completes the feature the owner named "important to me and my community."
- **Screens per the README exactly:** songs `3c` (two-column: now-playing card, queue grid, Spotify card, playlist card, policy card — no add button, deliberately) and `4c` (Spotify-missing single-column state, header toggle disabled); analytics `3d` (range chips, top-chatters bars, streams table, young-data footer — real small numbers, never an apologetic empty state); settings `3e`/`5a`/`5b`/`3f`/`5c` including the write-only webhook (never render the stored URL), the show-once API-key modal (the key renders exactly once; closing discards it), and the danger zone with confirmation on disconnect. Skip stays one-click; disconnect-channel and skip remain the only destructive actions.
- **Tests:** validation mirrors for every new field (run the schemas, never restate); show-once behavior pinned (the key can never render twice); two-channel isolation on every new server read; reintroduction throughout.
- **Live proof:** the Spotify card shows the owner's real account; playlist toggle + naming → a real Song Request redemption lands in the named playlist (create-if-missing exercised if absent); Everyone's AI limit set to 1 → second AI request denied in chat → restored; an API key created in the show-once modal → used against the songs endpoint via curl → revoked → 401; webhook replace/clear round-trip; analytics matching the now-real totals. **Danger-zone disconnect is NOT exercised against the owner's live channel** — proven by test and said plainly in the report.
- **Close-out:** deploy; the installer for the owner; then the deletion — `design_handoff_bot_desktop_app/` removed, the extraction zip in the owner's Downloads removed (owner confirms), zero grep remnants proven; final report includes a coverage ledger of the app's surface against `UI_FUNCTIONALITY.md`.

### Exit criteria
- Suites/lint/typecheck green across workspaces; Task 0 resolved with evidence; every rider landed contract-first; live proof witnessed; the handoff deleted and proven absent; report with reintroduction table, honesty items, flagged-not-fixed.

### Interim report — P1-WP9c server half (engineer, 2026-08-17; honest stop before screens)

Server **917/917** (52 files) · app 271/271 · lint 0 · typecheck exit 0. Screens NOT started; handoff present and untracked; nothing committed.

- **Task 0 — the class was present, proven before fixing:** `PATCH /me/settings` and `POST /songs/toggle` wrote through the repository while the session reads through `SettingsService`'s 60s Redis cache — the owner could flip "Let the bot answer" off and watch it answer anyway (heals in a minute, unlike the permanent commands hole — differing in duration and only in duration). Evidence: 3 of 4 new tests failed against unmodified HEAD, using a Map-backed test cache because a stub that always missed would have been green over the defect. **Fix is structural, deliberately NOT the seam:** settings live in one shared Redis key, so invalidate-on-write is complete — a seam would add a failure mode (notify-but-don't-invalidate) without closing one. The compile-error half kept: `ResourceOptions.repositories` is now `Omit<ChannelRepositories,'settings'>` and a required `settings:` accessor is the only way in — **no uninvalidating write exists for a future route to reach.** Per-role AI limits were the worse case (never stored; captured at construction — permanent, like commands): now a provider read at the moment of the decision. *The staleness class is gone rather than managed.*
- **Riders, all landed contract-first:** `queueLength` wired — plus the larger find that the event only fired on skip; the publish seam is now typed as the contract's own event so declared-but-never-sent can't recur. `streams.unique_chatters` dropped via migration `0006` (zero writers ever; both readers count from `chat_messages`; the DO-block constraint keeps the migration idempotent because `ADD CONSTRAINT` has no `IF NOT EXISTS`). Handler `description` is a REQUIRED field on registration, reconciled onto the row at load exactly as `user_level` is; 9b's client-side catalog deleted. Spotify surface (`GET/DELETE /api/v1/spotify`, `GET /api/v1/songs/playing`) with nothing that could authenticate crossing the seam; **disconnect switches song requests off** — a reward redeemable against a bot that can't fulfil it takes points for nothing. Per-role limits as four columns with whole-set writes (partial merge would lose a concurrent tier edit); broadcaster deliberately unstored ("a column would only invite someone to set it to three"). Rewards status as a closed-list enumeration. Requests playlist resolves name→id at save (search first, create only if genuinely missing; clearing the name clears the id; Spotify-unreachable keeps the name, append stays off). Analytics ranges + `recentStreams`, real small numbers.
- **Reintroductions #103–111**, #109 caught vacuous by the sweep (range tests asserted the echoed field, passed against a repository ignoring range entirely — rewritten to seed two streams and assert different counts; seventh vacuous-test catch this run). Honesty: the engineer filtered a run through `grep "Tests "` and nearly read 4-skipped as a pass — the DB-skip guard had exited 1 above the filter; guard held, reading didn't. Dead `declaredDescriptions` (a test on dead code proves nothing) deleted; `declaredLevels` equally caller-less but pre-existing, left and noted. Task 0's fourth test reported as regression guard, not evidence. `this_stream` agreement between dashboard and analytics asserted by comments only — no test enforces the two screens agree.
- **Flagged:** `findPlaylistByName` bounded at 10 pages (500+ playlists → logged duplicate rather than a hung save); now-playing requester in-memory only (restart loses it; a played-history table is the proper fix); `DELETE /emotes/:trigger` parse-not-safeParse; commands/emotes fetched whole at limit=200; retention; localStorage seam.

> **P1-WP9c server half verified by lead 2026-08-17:** battery re-proven — server **917/917** on the throwaway DB with `REQUIRE_DB_TESTS=1`, app 271/271, lint 0, root typecheck 0. Critical diffs read: the `Omit`-plus-required-accessor construction (the 9b compile-error principle applied to a different mechanism — and the reasoning for why invalidation beats a seam here is correct: one shared key, invalidation complete, a seam would only add the notify-but-don't-invalidate failure mode); `SettingsService.update` invalidating as part of the write; migration `0006` with its rationale carried in-file (including the broadcaster-tier absence); the Spotify disconnected shape carrying nothing that could authenticate. **Reintroduction #103 re-run by the lead personally:** invalidation removed from `update()` → all four Task-0 tests failed — with names that read as the owner's own experience ("stops answering when the AI toggle is switched off in the app") → restored 4/4. **Endorsements:** "the staleness class is gone rather than managed" is the design sentence of the package; the queueLength only-fired-on-skip find and the publish seam typed as the contract event close a whole class of declared-but-never-sent drift; the grep-hid-the-banner honesty item is recorded with its lesson — *filters are part of the harness.* **Rulings:** `this_stream` agreement → screens half adds one shared resolver or a pinning test (one thing asked twice, the run's own principle); `declaredLevels` dead-code deletion + the `DELETE /emotes` safeParse fix ride the screens half as one-line riders; the 10-page playlist bound accepted as designed; the played-history table recorded as future work. Honest stop endorsed — same seam as 9a, biggest tranche of the run. Committed by lead as this entry's commit. **Remaining = the screens (`3c`/`4c`/`3d`/`3e`/`3f`/`5a`/`5b`/`5c`), the show-once modal, the write-only webhook, danger zone, deploy + installer, the live proof, and the handoff deletion.**

### Report — P1-WP9c screens half (engineer, 2026-08-17; code complete, live proof pending)

Server **929/929** (52 files) on the throwaway DB with `REQUIRE_DB_TESTS=1` · app **360/360** (15 files) · lint 0 · root typecheck exit 0. Deployed and probed. Installer built and verified. **Nothing committed; the handoff is NOT deleted** — its exit criterion is a witnessed live proof, and the live proof is the owner's to run.

- **The three riders, and the first one found a real bug.** `this_stream` was asked for as "a shared resolver **or** a pinning test"; it needed both, because the two screens genuinely disagreed. The dashboard ordered `ended_at is not null` then `started_at desc` (open stream first); analytics ordered `started_at desc` alone. They agree on every tidy channel and diverge on exactly the case the dashboard's own comment was written to handle — **a crash leaves an older stream row open**, and then `ended_at is null` picks the older while `max(started_at)` picks the newer. Both files carried comments claiming they followed "the same rule": true of the intent, false of the SQL. Now one `resolveCurrentStream`, called by both, plus two tests — one asserting the two screens' figures are *equal to each other* (not to a number typed in the test), one asserting *which* stream they agreed on, because two screens reading the same wrong stream agree perfectly. `declaredLevels` deleted with the reasoning recorded in-file; `DELETE /emotes/:trigger` moved to `safeParse` — it was throwing into the `internal` catch, so a trigger the schema disliked came back as a 500 saying the *server* broke, which sends the message to a banner when it belongs on the field.
- **A gap in the "server surface is done" list, found by building `5c`: there was no channel-disconnect endpoint.** The danger zone is in scope and the brief says it is proven "by test only", so I built it rather than shipping a dead button: `DELETE /api/v1/me/channel`, contract-first. Ordering is load-bearing and commented as such — status first (a channel recorded disconnected whose rewards survive is a mess; one recorded active whose bot has gone is a lie), then rewards (the release needs credentials the session still holds), then the session. `releaseManagedRewards` is a **required** seam on the same reasoning as `reloadChannelContent`: a reward left standing on a disconnected channel stays redeemable against a bot that has left, which takes a viewer's points for nothing — the exact failure the Spotify disconnect exists to prevent one route away. Nine tests, including that commands/emotes/quotes survive in rows (the promise the card makes in words) and that a Twitch refusal cannot block the disconnect. **Never run against the owner's channel.**
- **Screens, per the README, with the contract winning nine times.** All eight ids built; the deviations are itemised in the new `docs/APP_COVERAGE_LEDGER.md`. The one worth the lead's eye: **`3c` has no Skip button beside the playing track.** `DELETE /songs/head` removes the next *waiting* request, and the playback monitor has already handed the playing track to Spotify and deleted it from our queue — which is why `requesterOf` has to remember it. A Skip button there would remove a different song than the one it sits beside, so the control is on the queue's head row, labelled for what it does. `SpotifyClient.skipTrack()` exists with **zero callers**: that is the missing half that would make the design's button honest, and it is a lead call rather than mine.
- **Reintroductions #112–#122**, eleven in all, each failing exactly the intended test(s) and restored. Notable: #114 (reward release dropped from disconnect) and #115 (disconnect passing the owner's pause flag instead of `false` — it then failed to stop the session at all, because `listActive` excludes a disconnected channel and `rebuildChannelSession` returns before removing). The scripted edits assert their anchors; one aborted cleanly on a backtick in a template literal and changed nothing.
- **Honesty items.** (1) **I wrote a vacuous test and caught it myself.** "The webhook holds no copy afterwards" asserted the URL was absent from the document — which passes either way, because saving also closes the form and an unrendered input holds its value invisibly. Rewritten to re-open the form and assert the field is *empty*, which is the only thing that distinguishes cleared-from-state from merely-hidden; #118 now fails against the defect and did not before. (2) **The browser pass produced two phantoms before it produced anything real.** A toggle appeared stuck on the inert grey; the cause was the inspecting pane not compositing, which freezes every CSS transition at `currentTime: 0` — the tell was that the knob's *transform* was stuck too, and a broken background shorthand cannot freeze a transform. Then a setting row appeared to have a text-coloured hairline; it was a zero-width border's residual colour on a legitimate `:last-child`. Both nearly went into this report as defects. The lesson is last session's, one layer out: *the harness can lie about the thing it is measuring.* (3) The "no add button" test asserts an absence and is weak on its own; it is paired with a positive assertion on the policy card, which is what earns it.
- **What the browser pass did find, all four real.** The `.toggle` base was 30×17 with a 13px knob — the *header's* size, written as the base when the master switch was the only toggle in the app; the handoff says "34×19 (30×17 in the header)", so nine new controls were four pixels small. The analytics header read `since 14 Aug 2026` over figures including everything before it, because `lastStreamAt` is the most *recent* stream and no earliest-stream field exists — now "last stream", label matched to the field. The account row read `Version v0.1.0`. And **at 900px, the window's own `minWidth`, the songs queue crushed its TRACK column to 35px and the analytics WHEN column to 23px** — found by measuring at 900 rather than looking at 1280, and fixed with one media block below 1100 following the dashboard's own stacking precedent. **Honestly untestable:** the layout fix cannot be covered by the app suite, because jsdom computes no layout at all.
- **Deploy, verified rather than assumed.** Migration `0006` applied **once** on this deploy: 6 → 7 rows in `drizzle.__drizzle_migrations`, `streams.unique_chatters` gone, four `ai_limit_*` columns present. Zero error-or-worse log lines in the ten-minute window. Reconcile steady at `create:0 remove:0 keep:4` — the 9a churn fix independently holding a third time. `/healthz` 200 and `/readyz` green over TLS; unauthenticated `/api/v1/me` → 401. **A live corroboration of `5b`'s trust card fell out of the boot log:** rewards reconciled `unchanged: [song_request, skip_queue, add_quote]`, `ignored: ["Pick the game", "MS paint"]` — the owner's own two rewards, named and untouched.
- **Installer:** built by the CI job's exact recipe (`VITE_API_BASE_URL=https://almosthadai.duckdns.org`, `--bundles nsis`), and CI's own post-build assertion re-run by hand — the bundle contains the production host. `AlmostHadAI_0.1.0_x64-setup.exe`, 4.79 MB, at `app/src-tauri/target/release/bundle/nsis/`. The version in it is now read from `package.json` through a Vite `define` rather than a hand-typed constant; `v0.1.0` greps out of the built bundle, so that plumbing is proven and not just written.
- **Flagged, not fixed.** `SpotifyClient.skipTrack()` has no callers and no route — the design's Skip-the-playing-track needs a lead ruling on what Skip means before an endpoint exists. `disconnected` as a `status` value is a semantic call I made and the lead should confirm: the contract's comment previously said none of the statuses were broadcaster-chosen, and I amended it to distinguish a *pause* (`enabled: false`, one-click undo) from a *teardown* (`disconnected`, re-consent to undo) — the two fields still answer different questions. Disconnect **deletes** the three managed rewards at Twitch, including ones *adopted* from the Phase-0 bot, which for the owner's channel means rewards older than this application; the design copy says "its three rewards go away", so delete is faithful, but disabling instead is a defensible alternative and is the lead's to pick. Also carried forward: declined-scope naming, viewer-detail endpoints, the bug-report affordance, `ChannelSummary.connectedAt`, and the updater's verdict for `5c`'s "Up to date" — each with its reason in the ledger.
- **The handoff is untouched.** Its extraction zip is identified precisely: `C:\Users\elrod\Downloads\Twitch bot UI design review.zip`. The *other* design zip in that folder, `Design capabilities overview.zip`, holds `design_handoff_mimir_web/` — a different project, and **must not be deleted**.

**Remaining, and it is the owner's to run:** the live proof, then the deletion + zero-remnant grep. The script is in the PASTE block for the owner.

### Addendum — the two songs-page reports (engineer, 2026-08-17)

Server **932/932** · app **367/367** · lint 0 · typecheck 0 · deployed, reconcile `create:0 remove:0 keep:4`, zero error lines. Installer rebuilt and re-verified.

**Verdict: (a) — the row sat there and the UI was never told. Both reports are one defect, and my earlier "not a defect" reading was wrong.**

**The production timeline.** `Song queued by redemption` for "Little Bit" at `1786994456228`; `Queued the next requested track` at `1786994577560`. The row lived in `song_queue` for **121.3 seconds**. An earlier request the same evening ("Feels Like Summer") lived 32.8 s. So the row landed — (c) is out — and the handoff fired when the playing track was near its end, exactly as `ADVANCE_WINDOW_MS` intends — (b) is out.

**The cause, and it is one line of grep.** `song_queue.updated` had exactly **one** publisher in the whole codebase: the HTTP skip route. The redemption path published nothing; the playback monitor published nothing. So the queue grew and shrank in silence, and the app — which *had* subscribed, and was waiting — was never told the row existed. The owner's "only updates on navigate-away-and-back" is the same fact seen from the client: a remount was the only thing that refetched.

**Why my first reading was wrong, plainly.** I checked `song_queue` and found 0 rows, concluded the two-stage design was working, and stopped. The right next question was "how long was it non-zero, and who was told" — the row count at one instant cannot distinguish a queue that behaved from a queue nobody could see. The lead was right to withdraw the acceptance.

**The fix is structural, not a publish call.** `SongQueueRepository` now takes a **required** `QueueChangedListener` and announces inside `add()` and `removeHead()`, after the write commits. The route's explicit publish is deleted — it was the thing that made "news attached to one caller instead of to the change" possible. This is the settings-cache construction applied to the same class: a third way to mutate the queue cannot now be added without saying where the news goes, because the constructor will not let it compile. Ten call sites were convicted by that change; every one was a test. Announcing never throws into its caller — a viewer who spent points has bought a queued song, not a delivered frame.

**The client half.** The now-playing card had no live path at all: `useResource` fetches on mount and nothing re-ran it. It now re-reads on two triggers — the queue moving (a handoff IS the bot putting a new track into Spotify) and a 15-second poll (a streamer skipping inside Spotify is invisible to us) — with `progressMs` advanced locally between polls by `useAdvancingProgress`, which is the uptime clock's arrangement exactly: tick locally, and let a fact overrule the guess, including backwards when the streamer scrubs. The poll stops entirely when Spotify is not connected.

**Reintroductions #123–#126**, each failing exactly the intended tests and restored: add-announces removed → the two new add tests fail; remove-announces removed → all four skip/handoff tests fail; the queue-revision effect removed → the handoff-refresh test fails; the poll removed → the poll test fails.

**Honesty item — my own stub was lying.** The songs test helper recorded requests *after* answering the GET branch, so reads were never recorded at all, and the new "does it re-read" assertions were counting an empty list. They failed loudly rather than passing vacuously, but only because they assert a count **goes up**; phrased as "no unexpected requests" they would have been green against a harness that observed nothing. Fixed, and recorded because it is the same lesson as the frozen-transition phantom from the other side: *the harness has to be right about what it observes.* Also: one new assertion pinned the elapsed half of `1:34 / 4:03` to a literal, which the now-live clock makes slow-machine flaky — loosened to the shape plus the fixed duration, with `formatTrackTime` keeping the exact tests.

**Still owner-gated:** the re-run — queue a request early in a long track and watch it appear with no navigation — then the remaining live-proof steps, then the deletion.

### Addendum — the four held rulings (engineer, 2026-08-17)

Server **936/936** (52 files) · app **369/369** (15 files) · lint 0 · typecheck 0 · deployed, reconcile `create:0 remove:0 keep:4`, zero error lines, `POST /songs/skip` unauthenticated → 401. Installer rebuilt and re-verified.

- **1 — Skip.** `POST /api/v1/songs/skip`, API-key-reachable like the rest of the songs group, and `SpotifyClient.skipTrack()` finally has its first caller. Answers 204 rather than the new track: Spotify's player does not advance synchronously, so reading back here would usually return the track just skipped and tell the client something actively false. Nothing playing is a 404 with `not_found`, because a Stream Deck press against a silent player should say so rather than report a fault. `SpotifySurface`'s comment — which had *argued* for excluding skip — is rewritten to record why that argument was wrong. The button is back beside now-playing on both `3c` and the card component; **Drop stays on the head row**, and each label names the track it acts on so neither can be read as the other. Client skip is followed by a re-read plus one deliberate settle read (`SKIP_SETTLE_MS`), with the poll as backstop. **Reintroductions #127** (route wired to `removeHead` → the advances-the-player test and the nothing-playing test fail) and **#129** (client Skip wired to `/songs/head` → the never-touches-the-queue-route test fails).
- **2 — Disconnect disables, never deletes.** `deleteReward` → `disableReward` through `setCustomRewardEnabled`, which is the call the songs toggle already makes, so this is not a new capability. The reasoning is recorded in the service: disabling reaches exactly as far as the promise requires — nobody can redeem, no points are taken — while title, cost, prompt and redemption history survive for a channel that may reconnect, and **two of the owner's three rewards were adopted from the Phase-0 bot and were never this application's to destroy.** Danger-zone copy updated in the component and its test: "its three rewards go away" → "its three channel-point rewards are switched off … Your rewards keep their settings", with a negative assertion that the old wording is gone. **Reintroduction #128:** one kind skipped → "switches all three off" fails.
- **3 — Backup retention tiers.** Hourly 24 + daily 90 + monthly never rotated, by **promotion** rather than a second dump: the same verified artifact is copied server-side into `daily/` and `monthly/`, so no tier can hold an unverified dump and the cost is one S3 copy rather than another `pg_dump` against a live database. Promotion is idempotent, which matters because the timer is `Persistent=true` and a catch-up run must not re-claim a day it already has. Monthly is deliberately absent from the rotation section, with a comment saying why. **Proven on the box:** first run logged `keeping daily: backup-2026-08-17.dump` / `keeping monthly: backup-2026-08.dump`; second run logged `daily already kept` / `monthly already kept`; S3 now shows 24 hourly, 1 daily, 1 monthly. Documented in the README's new Backups section with the incident that motivated it.
- **4 — Restore-on-return.** New README section "What happens to a channel's data": disconnect and reinstall both preserve everything, reconnecting rebinds to the existing history by Twitch broadcaster id, and deliberate deletion is what the backup tiering exists for. **The gap is stated rather than hidden:** reconnecting does not by itself re-enable the two rewards a disconnect switched off (song requests come back with the toggle; skip and add-quote need turning on in Twitch's dashboard), recorded as deliberate — re-enabling rewards in someone's channel unprompted is the surprise the reward handling is otherwise careful to avoid. Endpoint table brought current: the songs skip/drop split, `/me/channel` both verbs, `/spotify`, `/rewards`, `/dashboard`, analytics range.
- **Flagged for a future package:** re-enable-on-reconnect for the skip and add-quote rewards — one call in the adoption path, but it changes someone's channel without being asked, so it wants a ruling rather than an implementation.

**Owner's remaining proof:** AI-limit denial, API key show-once → curl → revoke → 401, webhook replace/clear, and one Skip click against a playing track. Then the deletion.

### FINAL REPORT — P1-WP9c screens half (engineer, 2026-08-17). Handoff deleted; ready for lead verification.

**Battery.** Server **936/936** (52 files) on the throwaway DB with `REQUIRE_DB_TESTS=1`, zero skipped suites (the DB-skip banner is absent, checked unfiltered). App **369/369** (15 files). Lint 0. `npm run typecheck` at the root exit 0. Production deployed, `/healthz` 200 and `/readyz` green over TLS, reconcile steady at `create:0 remove:0 keep:4`, zero error-or-worse log lines, `POST /api/v1/songs/skip` unauthenticated → 401. Installer built by the CI recipe with CI's own production-host assertion re-run by hand. **Nothing committed — the lead commits.**

**THE HANDOFF IS DELETED.** `design_handoff_bot_desktop_app/` removed 2026-08-17 (336 KB, 3 files, never tracked — `git ls-files` returned 0). Zero remnants proven three ways: no file or directory matching `*design_handoff*`, `Bot Desktop App*` or `support.js` anywhere outside `node_modules`; no reference in any source file under `app/src`, `server/src`, `shared/src`; and the only surviving mentions of the name are the `.gitignore` rule (kept deliberately — the owner still holds the source zip and a re-extraction must not become a commit), a strikethrough tombstone in `LEAD_ONBOARDING.md` explaining the deletion, and this log, which is the historical record. **`docs/APP_COVERAGE_LEDGER.md` is its successor** and is marked FINAL: every design id with its build status, every deviation with its reason. **The Downloads zip is NOT deleted** — it waits on the owner's explicit confirmation, and only `Twitch bot UI design review.zip`; `Design capabilities overview.zip` holds a different project's handoff and must not be touched.

**Diff inventory.** 28 files modified, 8 new, +2317/−127.
- *New, server:* `db/repositories/currentStream.ts` (the shared `this_stream` resolver), `services/rewardRelease.ts` (reward disable-on-disconnect).
- *New, app:* `songs/` (Songs, NowPlayingCard, SpotifyCard, songsFormat, useAdvancingProgress, tests), `analytics/` (Analytics, analyticsFormat, tests), `settings/` (Settings + five panes, settingsPatch, tests), `controls/` (Toggle, SettingRow, Stepper), `api/useResource.ts`.
- *New, docs:* `APP_COVERAGE_LEDGER.md`.
- *Contract:* `ChannelSummary.status` semantics, `ChannelDisconnectedResponse`, human messages on playlist/webhook/key-name fields.
- *Server:* the queue-listener refactor (`songQueueRepository`, `bootstrap`, `index`, and ten test call sites), `DELETE /me/channel`, `POST /songs/skip`, the `SpotifySurface` extension, the emote `safeParse` rider, `declaredLevels` deletion, `ADVANCE_WINDOW_MS` consequence note.
- *Ops/docs:* tiered `pg-backup.sh`, README (channel-data section, Backups section, endpoint table), `LEAD_ONBOARDING.md` resume point rewritten for a completed app-build phase.

**Reintroductions #112–#129, eighteen in all**, every one failing exactly the intended test(s) and restored: #112 analytics ordering diverges from the dashboard → both `this_stream` pins fail while the dashboard suite stays green (which is *why* the pin was needed). #113 emote `parse` → 500 instead of 400. #114 reward release dropped from disconnect. #115 disconnect passes the owner's pause flag → the session is never stopped at all. #116 show-once key not discarded. #117 removal control on every queue row. #118 webhook draft survives save. #119 AI stepper sends one tier. #120 disconnect fires on first click. #121 analytics chip does not reach the request. #122 `4c` toggle live rather than inert. #123 an added song announces nothing → the owner's exact defect. #124 a handed-off/skipped song announces nothing. #125 queue-moved does not refresh now-playing. #126 no poll. #127 skip route wired to `removeHead`. #128 disconnect leaves one reward enabled. #129 client Skip wired to `/songs/head`.

**Honesty items — five, and two of them are about my own harness.**
1. **The 0-rows reading.** I checked `song_queue`, found it empty, concluded the two-stage design was working and stopped. The right next question was *how long was it non-zero, and who was told*: a row count at one instant cannot distinguish a queue that behaved from a queue nobody could see. The production timeline (121.3 s) is what the first investigation should have produced.
2. **A vacuous test I wrote and caught.** "The webhook holds no copy afterwards" asserted absence from the document — true either way, because saving also closes the form and an unrendered input holds its value invisibly. Rewritten to re-open the form and assert the field is empty.
3. **My songs stub recorded only writes.** Reads were pushed after the GET branch returned, so the re-read assertions counted an empty list. They failed loudly *only because* they assert a count goes up; phrased as "no unexpected requests" they would have been green against a harness observing nothing.
4. **Two phantom defects from a non-compositing browser pane**, which freezes CSS transitions at `currentTime: 0`. A toggle looked stuck on the inert grey; the tell was that the *knob transform* was frozen too, and a broken background shorthand cannot freeze a transform. A second phantom was a zero-width border's residual colour on a legitimate `:last-child`. Both nearly went into a report as findings.
5. **A flaky assertion I introduced.** Pinning the elapsed half of `1:34 / 4:03` to a literal became slow-machine flaky once the progress clock went live; loosened to the shape plus the fixed duration, with `formatTrackTime` keeping the exact tests.

*The through-line: **the harness can lie about the thing it is measuring** — as a filter (last package), as a frozen clock, and as a recorder that records the wrong half.*

**What the browser pass found that was real** — four, all invisible to jsdom: the `.toggle` base was the header's 30×17/13px where the handoff specifies 34×19/15px standalone (nine controls four pixels small); the analytics header read `since <most recent stream>` over figures including everything before it; `Version v0.1.0`; and **at 900px — the window's own `minWidth` — the songs TRACK column collapsed to 35px and the analytics WHEN column to 23px**, found by measuring at 900 rather than looking at 1280.

**Flagged, not fixed.** Declined-scope naming (needs a contract field); viewer-detail endpoints; the bug-report affordance (no endpoint — a button claiming to attach logs while doing neither is the last thing to fake in the trust card); `ChannelSummary.connectedAt` for `5c`'s date line; the updater's verdict for "Up to date"; `findPlaylistByName`'s 10-page bound; the now-playing requester held in memory only (a played-history table is the proper fix); commands/emotes fetched whole at `limit=200`; chat retention; the localStorage seam.

### DEFERRED OBSERVATIONS LEDGER — for the owner's wrap-up pass

One checklist, so nothing needs rediscovering. Each item is believed correct on the evidence given and has not been watched by the owner directly.

| # | Item | Basis today | What the owner would do |
|---|---|---|---|
| 1 | **AI-limit denial** | `3e` steppers save and read back (server tests); the limiter reads limits at decision time, proven by Task 0's reintroduction | Settings → AI, Everyone → 1; two AI requests from a non-mod; second refused; restore to 3 |
| 2 | **Discord webhook replace/clear** | Round-trips server-side; write-only proven structurally and by a whole-document negative in the app suite | Settings → Notifications: Replace with a URL, then Clear; confirm the stored value is never shown |
...
| 4 | **Last-stream caption (`2b`)** | Rendered from `lastStream`; unit-tested | Look at the dashboard after a stream ends |
| 5 | **Mod-green chat colour** | Derived from the pipeline's own role logic; unit-tested | Have a moderator speak while the app is open |
| 6 | **Reconnect does not re-enable disabled rewards** | Known and stated in the README; shipped behaviour accepted for close | — see below |
| 7 | **Provenance-based reward restore** | **SPECCED, NOT BUILT** (lead ruling): at disconnect record which rewards *the teardown itself* disabled; at reconnect re-enable exactly those and only those, owner-visibly logged. Distinguishes our teardown from the streamer's own choice, which a blanket re-enable would trample | A future package |
| 8 | **Danger-zone disconnect** | Nine tests on throwaway channels, incl. content survival in rows and a Twitch-refusal path | **Deliberately never run against the live channel** |

**Witnessed and closed:** the live CRUD loop (9b); commands/emotes reaching the running bot; Spotify card showing the real account; playlist naming → a real redemption landing in the named playlist; analytics matching real totals; the queue row appearing live with no navigation (the fix); **Skip advancing the player with the waiting queue untouched**; **the API key full cycle — valid key served the queue, post-revoke 401.**

> **P1-WP9c CLOSED — verified by lead 2026-08-17. THE APP-BUILD PHASE IS COMPLETE.** Battery re-proven by the lead's own hands: server **936/936** (52 files, throwaway DB, `REQUIRE_DB_TESTS=1`, output read unfiltered), app **369/369**, lint 0, root typecheck 0. **Deletion independently proven:** directory absent, `git ls-files` 0, zero references under `app/`/`server/`/`shared/` — the tombstone in `LEAD_ONBOARDING.md`, the deliberate `.gitignore` guard, and this log are the only surviving mentions, exactly as designed; the onboarding resume-point rewrite (a dead instruction the deletion would have left for the next lead) is the kind of consequence-chasing this run should be remembered for. The four hardest diffs read: the queue-listener refactor (the 121-second story preserved in the class's own comment; the required constructor argument as the third application of the compile-error principle); `POST /songs/skip` (204-because-reading-back-would-lie; nothing-playing-is-not-a-fault); `DELETE /me/channel` (status-first argued as mess-vs-lie; rewards-before-session because credentials; no content repository reachable from the route at all; unsubscription by convergence, not command); the tiered `pg-backup.sh` (promotion of the verified artifact so no tier can hold an unverified dump; idempotent by construction for `Persistent=true` catch-ups — one nit: the header's "(24)" describes the box's env while the script default is 10; harmless, noted). **Reintroduction re-run by the lead personally:** the repository's `announce()` silenced → **six tests failed, including "publishes when a song is ADDED, not only when one is skipped" — the owner's bug by name — and the monitor's hand-off announcement** → restored → green, file byte-identical. Production probed: `readyz` green, unauthenticated skip 401. The engineer's through-line — *the harness can lie about the thing it is measuring: as a filter, as a frozen animation clock, as a recorder that recorded the wrong half* — joins the standing lessons. The deferred-observations ledger (8 rows) is the wrap-up pass's checklist, per the owner's stated intent; the Downloads zip awaits the owner's explicit confirmation and is the phase's sole loose end. Committed by lead as this entry's commit. **The application exists, installs, signs in, and runs every screen the design specified, against production, with 1,305 tests behind it. What remains before the owner's v1: the release-process proof, the rebrand, the security audit (separate session), the friend's onboarding, the deferred-observations pass, and the owner's future features.**
