...
Every entry in `package.json` is an exact version — `"express": "4.22.1"`, not
`"^4.22.1"`.

This is the standard for **applications**. Libraries use ranges so consumers can
dedupe; an application is the final consumer and gains nothing from that flexibility.
...
read of the changelog — that is why they are never grouped in with the rest.
...
- **Fix forward** — if the break is a small API change, push a commit onto Renovate's
  branch and merge once green.
...
- **Weekly schedule** — a batch on Monday morning beats a trickle all week.
- **Non-majors grouped, majors separate** — one PR to glance at, plus one PR per
  thing that genuinely needs thought.
...
- **`automerge: false` everywhere** — a human merges. On a project this size the
  review cost is seconds and the blast radius of an unattended bad merge is the whole
  bot.
- **`vulnerabilityAlerts` unscheduled** — security PRs ignore the Monday window and
  open immediately.
...
At the time of writing there are **2 moderate** advisories (`express` → `qs`). They
do not fail the build at the `high` threshold. They will clear on their own when
Express ships a bump, which Renovate will bring in.
...
See the WP-8 completion report for the click-through, or:
